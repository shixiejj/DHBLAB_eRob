...
void joint_move_UR()
{
    ...
    VectorXd target_point_joint(Number);      //目标位置,角度制
    VectorXd origin_point_joint_test(Number); //初始位置,角度制
    VectorXd vel_current_joint_test(Number);  //当前速度,角度制
    VectorXd acc_current_joint_test(Number);  //当前加速度,角度制
    double pos_cur_ang[Number];               //当前位置角度值,角度制
    ...
    vel_current_joint_test << 0, 0, 0; //设置当前速度
    acc_current_joint_test << 0, 0, 0; //设置当前加速度
    ...
    std::deque<double> trajectory_joint;        //得到的轨迹点
...
//---------------------------------------------------------------------------------------------------
 ...
 //---------------------------------------------------------------------------------------------------
    VectorXd pos_acs_original(Number); //初始位置,角度制
    for (int i = 0; i < Number; i++)
    {
        pos_acs_original(i) = g_UR->get_actual_position(i); //获取当前位置角度值
    }
    cout << "当前位置关节角:" << endl;
    ...
    /*    2023.7.24（加上变换末端姿态）      */
    string position;
    cout << "用户输入目标位置,格式: x|y|z|r|p|y|" << endl;          //RPY -> xyz 欧拉角  r->绕z轴角度，p->绕y轴角度,y->绕x轴角度
    cin >> position;
...
void eRob_position_read() {
    for(int i=0; i < Number; i++) {
        double position_temp = g_UR->get_actual_position(i);
        printf("%d轴位置:%f \n", i, position_temp);
    }
}
...
void joints_move_test_UR()
{
    ...
    string posIn;
    cout << "运动目标角度（-20°-20°），用户输入格式 pos0|pos1|pos2|" << endl;                       
    cin >> posIn;

    VectorXd target_pos(Number);   //移动目标角度
    int split_pos = -1;
    int i = 0;

    while (string::npos != (const long unsigned int)(split_pos = posIn.find("|")) and strcmp("|", posIn.c_str()) != 0)
    {
        double joint_position = atof(posIn.substr(0, split_pos).c_str());

		if (joint_position<-20 || joint_position>20)
		{
			cout<<"测试结束，关节测试退出！"<<endl;
			robot_is_moving = false;
			return;
		}

        target_pos(i) = joint_position; // x-y-z-r-p-y
        posIn = posIn.substr(split_pos + 1);
        i++;
        if (i==Number)
        {
            break;
	...
	VectorXd target_point_joint_test(Number);
	target_point_joint_test=target_pos;
    char joint_info[255];
    cout << "六个轴的转角:" << target_point_joint_test << endl;
    cout << "是否确定？yes|no" << endl;
    ...
    joints_move(target_point_joint_test);
    ...
    cout << "当前角度 deg:" << endl;
    VectorXd cur_angle(Number);
    for (int i = 0; i < Number; i++)
    {
        cur_angle(i) = g_UR->get_actual_position(i);
    }
	cout << cur_angle << endl;
...
void test_UR_func()
{
    ...
    dh.a[1]=243.5;
    dh.a[2]=0;
	dh.alpha[0] = M_PI * 90 / 180;
	dh.alpha[1] = M_PI * 0 / 180;
	dh.alpha[2] = M_PI * 0 / 180;
	dh.d[0] = 143.5;
	dh.d[1] = 86.5;
	dh.d[2] = 0;
	dh.theta[0] = M_PI * 0 / 180;
	dh.theta[1] = M_PI * 90 / 180;
	dh.theta[2] = M_PI * (-90) / 180;

	g_UR->set_DH_param(dh);
	...
	motor_pa.encoder.singleTurnEncoder[0] = 49660;
	motor_pa.encoder.singleTurnEncoder[1] = 462000;
	motor_pa.encoder.singleTurnEncoder[2] = 54779;
	motor_pa.encoder.singleTurnEncoder[3] = 0;
	motor_pa.encoder.singleTurnEncoder[4] = 0;
	motor_pa.encoder.singleTurnEncoder[5] = 0;
	...
	for(int i=0;i<6;i++)
	{
		...
		motor_pa.encoder.encoderResolution[i] = 524288;
...
int main()
{
    ...
    while(status_test[0] == 0 || status_test[1] == 0 || status_test[2] == 0)
    {
        sleep(1);
    }
    printf("get status! \n");
